...
check_ingredients = input('Would you like to add any others? (Y/[N]): ')
if (check_ingredients == 'Y') or (check_ingredients == 'y'):
	ingredients = (ingredients_input + ', ' + check_ingredients).split(',')
else:
...
def find_recipe(your_ingredients, recipe_bank):
	...
	ingredients_matches = [len(np.where(np.isin(your_ingredients, i.split(',')))[0]) for i in ingredients_bank]
	best_match = np.argmax(ingredients_matches)
	print('Based on your ingredients, we suggest you cook',recipe_bank.recipe_name.values[best_match])
	print('You have {}/{} ingredients already.'.format(ingredients_matches[best_match], len(ingredients_bank[best_match].split(','))))
